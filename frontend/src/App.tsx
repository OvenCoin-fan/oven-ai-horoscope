...
function App() {
  ...
  return (
    <div className="app">
            ...
            <div className="card">
                  ...
                  <div style={{padding:'12px',background:'rgba(212,160,23,0.06)',borderRadius:'12px',marginBottom:'12px'}}>
                    <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'8px'}}>
                      ...
                      <div style={{display:'flex',alignItems:'center',gap:'6px'}}>
                        <input type="number" placeholder="0" value={swapAmt} onChange={e => setSwapAmt(e.target.value)} min="0" step="any" style={swapInputStyle} />
              ...
              {tab === 'stake' && (
                <>
                  ...
                  <input type="number" placeholder="Сумма $OVEN" value={stakeAmt} onChange={e => setStakeAmt(e.target.value)} style={inputStyle} />
                      ...
                      {stakes.filter(s => !s.claimed).map(s => {
                        ...
                        return (
                          <div key={s.id} style={{padding:'12px',background:'rgba(212,160,23,0.04)',borderRadius:'10px',border:'1px solid rgba(212,160,23,0.12)',marginBottom:'8px'}}>
                            <div style={{display:'flex',justifyContent:'space-between',marginBottom:'4px'}}>
                              <span style={{fontSize:'13px',color:'#e8e8e8'}}>{tier.emoji} {tier.label} &bull; {s.apy}% APY</span>
                  ...
                  <div key={mk.id} style={{marginBottom:'20px',padding:'16px',background:'rgba(212,160,23,0.04)',borderRadius:'12px',border:'1px solid rgba(212,160,23,0.12)'}}>
                    ...
                    {t && t.outcome !== undefined && (
                      <div style={{padding:'10px',background:'rgba(212,160,23,0.06)',borderRadius:'8px'}}>
                        ...
                        <input type="number" placeholder="Введите количество" value={t.amount} onChange={e => setTrade(p => ({...p,[mk.id]:{...p[mk.id],amount:e.target.value}}))} min="0" step="any" style={inputStyle} />
                        {shareNum > 0 && (
                        ...
                        <div style={{display:'flex',gap:'6px'}}>
                          ...
                          {(myPos[t.outcome]||0) > 0 && (
                            <button onClick={() => sellShares(mk.id,t.outcome,Math.min(shareNum,myPos[t.outcome]||0))} disabled={shareNum<=0} style={{flex:1,padding:'8px',borderRadius:'8px',border:'1px solid rgba(255,50,50,0.4)',background:'rgba(255,50,50,0.1)',color:'#f88',fontSize:'13px',cursor:shareNum>0?'pointer':'not-allowed'}}>Продать {Math.min(shareNum,myPos[t.outcome]||0)}</button>
                          )}
                        </div>
                      </div>
                    )}
      ...
      <footer className="footer"><p>🔥 $OVEN AI Horoscope &bull; Built on TON &bull; Powered by Mira</p></footer>
    </div>
  );
}
