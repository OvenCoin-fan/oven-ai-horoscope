    ...
    <>
      <div className="card">
        <div className="balance-bar" style={{ marginBottom: '0' }}>
          <div><span className="label">🔥 $OVEN</span><span className="amount" style={{ marginLeft: '8px' }}>{wallet.ovenBalance}</span></div>
          <div><span className="label">💎 TON</span><span className="amount" style={{ marginLeft: '8px' }}>{wallet.tonBalance}</span></div>
        </div>
      </div>
      ...
      <div className="card">
        ...
        <button className="mint-btn" onClick={() => alert('В продакшене: jetton transfer через TonConnect')}>
          🔥 Минт 100 $OVEN
        </button>
      </div>
              ...
              {trade !== undefined && trade.outcome !== undefined && (
                <div style={{ padding: '10px', background: 'rgba(212,160,23,0.06)', borderRadius: '8px' }}>
                  ...
                  <div style={{ marginBottom: '8px', padding: '8px', background: 'rgba(212,160,23,0.08)', borderRadius: '6px' }}>
                    <p style={{ fontSize: '12px', color: '#888' }}>Стоимость: <span style={{ color: '#e8e8e8' }}>{getCost(mk.pool, trade.outcome, trade.amount).toFixed(2)} $OVEN</span></p>
